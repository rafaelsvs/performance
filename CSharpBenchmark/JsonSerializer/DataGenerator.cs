...
namespace CSharpBenchmark.JsonSerializer
{
    internal static class DataGenerator
    {
        ...
        private static MyEventsListerViewModel CreateMyEventsListerViewModel()
            ...
            {
                CurrentEvents = Enumerable.Repeat(CreateMyEventsListerItem(), 300).ToList(),
                FutureEvents = Enumerable.Repeat(CreateMyEventsListerItem(), 900).ToList(),
                PastEvents = Enumerable.Repeat(CreateMyEventsListerItem(), 6000).ToList() // usually  there is a lot of historical data
            };
